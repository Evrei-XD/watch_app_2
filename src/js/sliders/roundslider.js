...
(function (factory) {
  ...
  RoundSlider.prototype = {
    ...
    _appendTooltip: function () {
      ...
      var tooltip = (this.tooltip = this.$createElement(
    ...
    _editTooltip: function (e) {
      ...
      var input = (this.input = this.$createElement(
    ...
    _setRadius: function () {
      ...
      if (extraSize !== 0)
        this.innerContainer.css({ height: actualHeight, width: actualWidth });
    ...
    _defaultValue: function () {
      ...
      var defaultValue = this.isNumber(startValue)
        ? this._limitValue(startValue)
        : o.min;
      return defaultValue;
    },
        ...
        if (distance >= innerDistance && distance <= outerDistance) {
          ...
          if (this._rangeSlider) {
            if (handle.length == 1) {
              ...
              this._active =
                this._handle2.angle - angle < angle - this._handle1.angle
                  ? 2
                  : 1;
    ...
    _changeSliderValue: function (value, angle) {
      ...
      if (!this._showRange) {
        ...
        var isValidRange =
          (activeHandle == 1 &&
            oAngle <= this._oriAngle(this._handle2.angle)) ||
        ...
        if (this._minRange || isValidRange || canAllowInvertRange) {
          this['_handle' + activeHandle] = { angle: angle, value: value };
          options.value = this._rangeSlider
          ...
          (activeHandle == 1 ? this.block4 : this.block2).rsRotate(
            lAngle - 180
          );
    ...
    _createSVGElements: function () {
      ...
      this.$range = this._showRange
        ? this.$createSVG(PATH + 'rs-range', pathAttr)
        : null;
    ...
    _moveSliderRange: function (isInit) {
      ...
      var handle2Distance =
        ((handle2Angle - handle1Angle) / totalAngle) * this.svgPathLength;
    ...
    _processStepByValue: function (value) {
      ...
      if (!isMinHigher) {
        if (value >= currVal)
          newVal = value - currVal < nextVal - value ? currVal : nextVal;
        else newVal = currVal - value > value - preVal ? currVal : preVal;
      } else {
        if (value <= currVal)
          newVal = currVal - value < value - nextVal ? currVal : nextVal;
    ...
    _limitValue: function (value) {
      var o = this.options,
        ...
        max = o.max,
        isMinHigher = min > max;
      if ((!isMinHigher && value < min) || (isMinHigher && value > min))
        value = min;
      if ((!isMinHigher && value > max) || (isMinHigher && value < max))
        value = max;
      return value;
    },
    ...
    _appendHiddenField: function () {
      var hiddenField = (this._hiddenField =
    ...
    _tooltipValue: function (value, index) {
      ...
      return returnValue != null && typeof returnValue !== 'boolean'
        ? returnValue
        : value;
    },
    ...
    _set: function (property, value, forceSet) {
      ...
      switch (property) {
        ...
        case 'readOnly':
          options.readOnly
            ? this._readOnly(true)
            : !options.disabled && this._readOnly(false);
          break;
        case 'mouseScrollAction':
          this._bindScrollEvents(
            options.mouseScrollAction ? '_bind' : '_unbind'
          );
          break;
        case 'lineCap':
          this._setRadius();
  ...
  // ### SVG related logic
  RoundSlider.prototype.$polarToCartesian = function (
    centerXY,
    radius,
    angleInDegrees
  ) {
  ...
  RoundSlider.prototype.$drawArc = function (
    centerXY,
    radius,
    startAngle,
    endAngle,
  ...
  RoundSlider.prototype.$drawPath = function (
    centerXY,
    outerRadius,
    startAngle,
    endAngle,
